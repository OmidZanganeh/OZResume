...
import { RETURN_PERIOD_OPTIONS, returnPeriodHint } from './returnPeriods';
...
interface Props {
  ...
  onReturnPeriodChange?: (periodDays: number) => void;
...
export default function DateTimeline({
  ...
  const showReturnPeriod = daysAgo > 0 && onReturnPeriodChange != null;
    ...
    <div className={styles.dateBar}>
      ...
      {showReturnPeriod && (
        ...
        <div className={styles.dateReturnRow}>
          ...
          <label className={styles.dateReturnLabel} htmlFor="return-period">
          ...
          <select
            ...
            aria-label="Return period forward from screen-as-of date"

          >

            {RETURN_PERIOD_OPTIONS.map(opt => (

              <option key={opt.id} value={opt.days}>
          ...
          <span className={styles.dateReturnHint}>
            ...
            {returnPeriodHint(displayDaysAgo, returnPeriodDays)}
