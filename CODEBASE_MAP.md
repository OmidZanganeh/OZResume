...
| `stock-screener` | Finnhub — S&P 500 + NASDAQ 100 + S&P 400 (`?universe=`); incremental Redis cache per index; cron Sundays 05:00–07:30 UTC |
...
| Routine run exercise swap | `RoutineRunView.tsx` + `style.css`: **Use today** = `sessionOrderIds`; **Update plan** = mutate `savedPlans[].exerciseIds`; preset **Save to My plans** = fork. Swap has two modes: **Same muscles** (top-12) and **Browse all** (searchable full catalog, 60 shown). |
| Routine run draft persistence | `RoutineRunView.tsx`: drafts saved to **localStorage** with a 36 h TTL `{savedAt, payload}` wrapper — survives screen-lock/PWA reload. Falls back to sessionStorage on quota error. |
| Quick Workout tab | `App.tsx` Plans sub-tab `'quick'`: body-map muscle-selector → filtered exercise checklist → **Start** creates ephemeral `SavedPlan` (`gf-quick-active`) and opens RoutineRunView. State: `quickMuscles`, `quickPickedIds`, `quickSearch`, `quickFilteredExercises`. |
| Plan image export | `utils/exportPlanImage.ts` + `utils/openImageInBrowser.ts` + **Download** on plan cards. Opens PNG in a browser tab (native save/share). Summary **Open image** uses the same open-in-tab behavior. |
| Edit plan screen | `App.tsx` view `edit-plan`: dedicated single-screen editor (name, ordered exercises + muscle targets, Add exercises catalog). New Plan wizard unchanged (`create-focus` → `create-moves`). |
