...
| Plan image export | `utils/exportPlanImage.ts` + **Preview** on plan cards (My plans, Browse presets, muscle suggestions). Opens the same in-app image preview modal as Summary (**Save image** / Cancel). Tall PNG via `html-to-image`. |
