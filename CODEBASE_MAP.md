...
| Plan image export | `utils/exportPlanImage.ts` + **Download** on plan cards (My plans, Browse presets, muscle suggestions). Tall PNG via `html-to-image`; uses Web Share when available, else file download. Includes exercises, muscles, equipment, last logged sets/reps/weight. |
