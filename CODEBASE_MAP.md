...
| Exercise muscle groups | `Gym Webapp/src/data/exerciseTypes.ts` + `Gym Webapp/src/data/exerciseLibrary.ts` + `Gym Webapp/src/bodyMap/bodyMusclesMapping.ts`: canonical group list and smart remapping of wrkout move names into expanded groups (`Adductors`, `Abductors`, `Tibialis`, `Obliques`, `Hip Flexors`, `Neck`) with body-map IDs where available. |
